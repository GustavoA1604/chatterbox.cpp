...
// 202 (attached below), 216 (attached above). We use a lightweight map covering
// all combining marks present in our tables.
static int combining_class(uint32_t cp) {
    if (cp < 0x0300) return 0;
    if (cp >= 0x0300 && cp <= 0x0314) return 230;
    if (cp >= 0x0315 && cp <= 0x0315) return 232;
    if (cp >= 0x0316 && cp <= 0x0319) return 220;
    if (cp >= 0x031A && cp <= 0x031A) return 232;
    ...
    if (cp >= 0x0346 && cp <= 0x0346) return 230;
    if (cp >= 0x0347 && cp <= 0x0349) return 220;
    if (cp >= 0x034A && cp <= 0x034C) return 230;
    if (cp >= 0x034D && cp <= 0x034E) return 220;
    ...
    if (cp >= 0x035C && cp <= 0x035C) return 233;
    if (cp >= 0x035D && cp <= 0x035E) return 234;
    if (cp >= 0x035F && cp <= 0x035F) return 233;
    ...
    if (cp == 0x05C1) return 230;
    if (cp == 0x05C2) return 220;
    if (cp == 0x05C4) return 230;
    if (cp == 0x05C5) return 220;
    if (cp == 0x05C7) return 220;
    if (cp == 0x0610) return 230;
    ...
    if (cp == 0x06DF) return 230;
    if (cp == 0x06E0) return 230;
    if (cp == 0x06E1) return 230;
    if (cp == 0x06E7) return 230;
    if (cp == 0x06E8) return 230;
...
bool mtl_tokenizer::is_language_supported(const std::string & lang) const {
    for (const auto & s : k_supported) if (s == lang) return true;
    return false;
}
